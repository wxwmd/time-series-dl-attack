...
def clip_eta(eta, norm, eps):
    """
    PyTorch's implementation of the clip_eta in utils_tf.
    :param eta: Tensor
...
def get_or_guess_labels(model, x, **kwargs):
    ...
    The kwargs are fed directly from the kwargs of the attacks.
    If 'y' is in kwargs, then assume it's an untargeted attacks and
    use that as the label.
    If 'y_target' is in kwargs and is not none, then assume it's a
    targeted attacks and use that as the label.
    Otherwise, use the models's prediction as the label and perform an
    untargeted attacks.
    :param model: PyTorch models. Do not add a softmax gate to the output.
